...
def _serialize_file(item: SelectionCandidateFile) -> dict:
    return {
        "id": item.id,
...
def _serialize_candidate(candidate: SelectionCandidate, *, include_label: bool = False, labeled_root: Path | None = None) -> dict:
    payload = {
        "id": candidate.id,
        ...
        "files": [_serialize_file(item) for item in candidate.files],
    }
...
def list_directories(path: str = Query(default="")) -> dict:
    relative = path.strip().replace("\\", "/").strip("/")
    try:
        current = settings.data_root_path.resolve() if not relative else resolve_directory(settings.data_root_path, relative)
    except ValueError as exc:
    ...
    return {
        ...
        "root_host_path": settings.host_data_root_path,
...
def list_candidates(
    ...
    rows = db.scalars(
        ...
        .order_by(SelectionCandidate.updated_at.desc(), SelectionCandidate.id)
        .offset((page - 1) * page_size)
        .limit(page_size)
    ).all()
    return {
        "results": [_serialize_candidate(row) for row in rows],
        "count": count,
        "page": page,
        "page_size": page_size,
